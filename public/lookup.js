  ...
  if (lookupForm) {
    lookupForm.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        document.getElementById("itinerary-id").textContent =
          `Confirmation Code: ${result._id}`;
        document.getElementById("room-name").textContent = result.room.name;
        document.getElementById("room-img").src =
          result.room.images && result.room.images[0]
            ? "/" + result.room.images[0]
            : "/assets/blue-room.jpg";
        document.getElementById("check-in-val").textContent = checkInFormatted;
        document.getElementById("check-out-val").textContent =
          checkOutFormatted;
        document.getElementById("guest-name-val").textContent =
          result.guestName;
        document.getElementById("total-price-val").textContent =
