...
exports.lookupBooking = async (req, res) => {
  try {
    ...
    if (!bookingId || !email) {
      return res
        .status(400)
        .json({ message: "Both Booking ID and Email are required." });
    }
    ...
    if (!booking) {
      return res
        .status(404)
        .json({ message: "No reservation found matching those details." });
    }
    ...
    if (error.name === "CastError") {
      return res
        .status(400)
        .json({ message: "Invalid Confirmation ID format." });
    }
    res.status(500).json({ message: error.message });
  }
};
